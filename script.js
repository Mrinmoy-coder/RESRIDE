...
function processRide(rideType) {
    ...
    if (!startSub || !endSub) { alert("Select Path!"); return; }

    // 2. LIVE TRAFFIC & AI PRICING
    ...
    const driver = matchBestDriver();

    // 3. 3D & AR SIMULATION TRIGGERS
    isTripActive = true;
    ...
    document.getElementById('viewport-3d').classList.add('spatial-view');
    ...
    setTimeout(() => {
        ...
        receiptTimer = setTimeout(() => {
            if(isTripActive) {
                ...
                isTripActive = false;
                document.getElementById('viewport-3d').classList.remove('spatial-view');
    ...
    log.innerHTML = `<div style="margin-bottom:20px; border-left:3px solid ${rideType === 'Emergency' ? '#ff0055' : '#38bdf8'}; padding-left:10px;">
        ...
        <p style="color: #0ff;">> ETA: ${pWait} MINS</p>
    </div>` + log.innerHTML;
}
...
function submitRating(stars, from, to, type) {
    rideHistory.unshift({ time: new Date().toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}), from, to, fare: currentFare, type });
...
function rechargeWallet() { let amt = prompt("Amount:"); if(amt) { wallet += parseInt(amt); localStorage.setItem('resrideWallet', wallet); updateUI(); } }
